...
class GetTally(webapp.RequestHandler):
    def get(self):
        try:
            ...
            placeMaps = []
            placeMaps.extend([pm for pm in cityPMs if pm.RegionSeriesName and pm.RegionNameSource])
            ...
            placeMaps.extend([pm for pm in statePMs if pm.RegionSeriesName and pm.RegionNameSource])
            ...
            placeMaps.extend([pm for pm in ctryPMs if pm.RegionSeriesName and pm.RegionNameSource])

            #for each PlaceMap, look up Tally
            self.response.out.write('<Indexes>')
            for placeMap in placeMaps:
                #self.response.out.write('Name=%s, RSN=%s, RNS=%s, TCN=%s\n' % (placeMap.Name, placeMap.RegionSeriesName, placeMap.RegionNameSource, self.request.get('tallyID')))
                tallies = Tally.all().filter('RegionSeriesName =', placeMap.RegionSeriesName).filter('RegionNameSource =', placeMap.RegionNameSource).filter('TallyClassName =', self.request.get('tallyID'))
                ...
                for dp in TallyDataPoint.all().filter('Tally_ID =', tally.key()):
                    # <Indice date="2002-03-01T00:00:00Z" value="75.9" />
                    self.response.out.write('<Indice date="%sT00:00:00Z" value="%s" />\n' % (dp.Date.strftime("%Y-%m-%d"), dp.IxVal))
